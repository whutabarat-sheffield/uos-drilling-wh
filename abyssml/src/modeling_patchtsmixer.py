...
class PatchTSMixerForPredictionHead(nn.Module):
    ...
    def __init__(self, config: PatchTSMixerConfig, distribution_output=None):
        ...
        if distribution_output is None:
            ...
            self.base_forecast_block1 = nn.Linear(config.prediction_length, config.prediction_length)
            self.base_forecast_block2 = nn.Linear(config.prediction_length, config.prediction_length)
        else:
    ...
    def forward(self, hidden_features):
        ...
        # Modified here!!!!!
        forecast = self.base_forecast_block1(forecast)
        forecast = self.base_forecast_block2(forecast)
...
def random_masking(
    ...
    #for ze in range(inputs_mask.shape[2]):
    #	print('inputs_mask', inputs_mask[0][0][ze][0])
...
def forecast_masking(
    ...
    inputs_mask = inputs.masked_fill(mask.bool(), mask_value)
    #print('mask shape', inputs_mask.shape)
    return inputs_mask, mask[..., 0]
...
class PatchTSMixerForPretraining(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        if self.masked_loss is True:
            ...
            loss_power = torch.nn.MSELoss(reduction="none") # Use the Thrust power as loss (For Drilling Only!!!)
            loss_var = torch.nn.MSELoss(reduction="none") # Use the Variance as loss (For Drilling Only!!!)
        else:
            loss = torch.nn.MSELoss(reduction="mean")
            loss_power = torch.nn.MSELoss(reduction="mean") # Use the Thrust power as loss (For Drilling Only!!!)
            loss_var = torch.nn.MSELoss(reduction="mean") # Use the Variance as loss (For Drilling Only!!!)
        ...
        if return_loss is True:
            ...
            # For calculate Integra (Drilling power) Loss
            target_power = torch.sum(torch.abs(x_hat), dim=3) / 10 #x_hat.shape[-2]
            input_power = torch.sum(torch.abs(model_output.patch_input), dim=3) / 10 #x_hat.shape[-2]
            # For calculate Variance Loss in dim 3
            target_var = torch.var(x_hat, dim=3) 
            input_var = torch.var(model_output.patch_input, dim=3) 


            loss_p = loss_power(target_power, input_power)
            loss_v = loss_var(target_var, input_var)
            
            if self.masked_loss is False:
            	loss_val = loss(x_hat, model_output.patch_input) + loss_p + loss_v
            ...
            #print('loss_p', loss_p)
            #print('loss_v', loss_v)
            #print('loss_val', loss(x_hat, model_output.patch_input))
        ...
        if self.masked_loss is True and loss_val is not None:
            loss_p = (loss_power(target_power, input_power) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10)

            loss_v = (loss_var(target_var, input_var) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10)
            loss_val = (loss_val.mean(dim=-1) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10) + loss_p + loss_v
            #print('loss_p', loss_p)
...
class PatchTSMixerForPrediction(PatchTSMixerPreTrainedModel):
    ...
    def __init__(self, config: PatchTSMixerConfig):
        ...
        self.focal_loss = FocalLossForHeatmaps(alpha=0.75, gamma=2.0)
        self.use_return_dict = config.use_return_dict
        self.prediction_channel_indices = config.prediction_channel_indices
        ...
        # self.head_offset = PatchTSMixerOffsetHead(
        #     config=config,
        #     distribution_output=self.distribution_output,
        # )
        # self.head_reg = PatchTSMixerLinearHead(
        #     config=config,
        #     distribution_output=self.distribution_output,
        # )
    ...
    def forward(
        ...
        """
        # the reduction of loss has been modified from mean to none, in order to compute the weight of loss.
        if self.loss == "mse":
        ...
        #y_hat_offset = self.head_offset(model_output.last_hidden_state)
        # print('model_output.last_hidden_state', model_output.last_hidden_state.shape)
        # print(y_hat_offset.shape)
        ...
        if self.prediction_channel_indices is not None:
            ...
            if self.distribution_output:
                ...
                if future_values is not None and return_loss is True:
                    ...
                    loss_val = weighted_average(loss_val)
            # Training calls here!!! below
            else:
                y_hat = y_hat * model_output.scale + model_output.loc
                # 加入另外的输出

                # y_hat_out = [y_hat, y_hat_offset]
                if future_values is not None and return_loss is True:
                    # print('HEY!!!!!')
                    #loss_val = loss(y_hat, future_values)
                    loss_val = self.focal_loss(y_hat, future_values)
                    #print('**********************')
        ...
        return PatchTSMixerForPredictionOutput(
            loss=loss_val,
            #prediction_outputs=y_hat_out,  # tensor [batch_size x prediction_length x num_input_channels]
...
# Here is the focal loss implementation


class FocalLossForHeatmaps(nn.Module):
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        super(FocalLossForHeatmaps, self).__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, inputs, targets):
        ...
        # 定义pt为预测与目标之间的接近程度
        pt = torch.exp(-mse_loss)  # 近似错误的逆指示
...
# here is the offset head
class PatchTSMixerOffsetHead(nn.Module):
    """Linear head for Classification and Regression.
